<?php
    ...
    foreach($properties as $property){
        if($property['statut'] !="online"){
            array_push($removedProperties,$property);
        }
    }
    //Hide
    if(isset($_POST['hideButton'])){
        $actionID = $_POST['property_id'];
        $actionID = mysqli_real_escape_string($db,$actionID);
        $query = "UPDATE properties SET `statut` = 'offline' WHERE `properties`.`id` = $actionID";
    ...
    if(isset($_POST['showButton'])){
        $actionID = $_POST['property_id'];
        $actionID = mysqli_real_escape_string($db,$actionID);
        $query = "UPDATE properties SET `statut` = 'online' WHERE `properties`.`id` = $actionID";
        mysqli_query($db,$query);
        //Redirect
        header('Refresh: 0');
    }
    ...
    if(isset($_POST['deleteButton'])){
        ...
        //Delete property
        $actionID = mysqli_real_escape_string($db,$actionID);
        $query = "DELETE FROM properties WHERE `id` = $actionID";
        mysqli_query($db,$query);
        //Redirect
        header('Refresh: 0');
    }
...
<head>
    ...
    <style>
        ...
        .bubble_container[data-active="true"]{
            display: flex;
        }
...
<body>
        ...
        <div class="userpage_content">
            <div class="profile">
                ...
                <div class="container">
                    <div class="column">
                        <label>Nom</label>
                        <span><?php echo htmlspecialchars($userData['nom'])?></span>
                    </div>
                    <div class="column">
                        <label>Prénom</label>
                        <span><?php echo htmlspecialchars($userData['prenom'])?></span>
                    </div>
                    <div class="column">
                        <label>Téléphone</label>
                        <span><?php echo htmlspecialchars($userData['numero'])?></span>
                    </div>
                    <div class="column">
                        <label>Email</label>
                    ...
                    <div class="column">
                        <label>Pays</label>
                        <span><?php echo htmlspecialchars($userData['pays'])?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="properties" oncontextmenu = "return false;">
                        ...
                        <?php foreach($properties as $property):?>
                            <?php if($property['statut'] == "online"):?>
                                <div class="property_box" data-long="true">
                                    ...
                                    <form class="bubble" method="post" action="userpage.php">
                                        <input class="hidden" value="<?php echo $property['id']?>" name="property_id">
                                        ...
                                        <div class="bubble_icon deleteBtn" data-action="deleteBtn">
                                    ...
                                    <a href="property_page.php?location=userpage.php&id=<?php echo htmlspecialchars($property['id'])?>">
                                        <img src="
                                        <?php
                                            $images = explode(',',$property['images']);
                ...
                ?>
                <h2>Propriétés Retirées</h2>
                <div class="properties_container">
                <?php foreach($removedProperties as $property):?>
                    <div class="property_box" data-long="true">
                                <div class="bubble_container">
                                <form class="bubble" method="post" action="userpage.php">
                                        <input class="hidden" value="<?php echo $property['id']?>" name="property_id">
                ...
                <?php endif?>
                <?php if(mysqli_num_rows($properties)>3):?>
                    <label for="" class="more"><a href="properties.php">
    ...
    let timmy = null;
    let timmyLong = null;
    const delay = 300; //ms delay to be considered a long press
    ...
    //remove all the flyout buttons after delay if no interaction for 2.5 seconds
    timmyLong = setTimeout(resetButtons.bind(btn), 250000);

   
    btn.children[0].dataset.active = true;
    }

    function resetButtons() {
    let bubble_container = this.children[0];
    delete bubble_container.dataset.active;
    addClickTouch();
    }
    //Longpress end
    //Delete popup
    ...
    bubbles.forEach(bubble=>{
        bubble.addEventListener('click',(e)=>{
            if(e.target.closest(".deleteBtn")){
                ...
                //remove once user clicks cancel or outside
                document.addEventListener('click',(e)=>{
                    ...
                    if(e.target.closest('.cancel') || e.target.closest('.screen_overlay')){
                        if(!e.target.closest('.text_container') && !e.target.closest('.deleteButton')){
                            try{
                                bubbleContainer.removeChild(popup);
                            }catch(e){}
                        }
                    }
                })
