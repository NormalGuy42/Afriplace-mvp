<?php 
    ...
    $previousLocation = $_GET['prev'];

    $headerLink = $location.'?location='.$previousLocation.'&id='.$propertyID;
    ...
    if(isset($_POST['submit'])){
        ...
        if(!empty($_POST['quirks'])){
            if(array_filter($_POST['quirks'])){
                $GLOBALS['quirks'] = implode(',',$_POST['quirks']);
            }
        }
        //Image Upload start
        ...
        if(!array_filter($errors)){
            ...
            $sql = "UPDATE properties SET `title`='$title', `price`=$price,`type`= '$type', `action`='$statut', `bedNum`=$bedNum,
            `toiletteNum`= $toiletteNum , `surface`= $surface, `description`= '$description' , `quirks`= '$quirks', 
            `images`= '$images', `localisation`= '$localisation', `userID`= $userID,`quartier`= '$quartier' ,`communes urbaines`= '$district' , `prefectures`= '$prefecture', `regions` ='$region', `pays` ='$country' WHERE id=$propertyID";
    ...
    <!--Header-->
        <form class="create_listing" enctype="multipart/form-data" method="POST">
            ...
            <div class="image_send">
               ...
               <input type="file" multiple accept="image/*" id="upload" name="image[]" value="">
               <label for="upload" name="uploadBtn">
               ...
               <div class="btn_input <?php if($type == "Maison"|| $type=="Appart"){echo 'showSection';}?>" id="house_stats">
                        <div class="container">
                            ...
                            <div class="custom_btn" id="bed_num">
                                ...
                                <span id="num_lits"><?php echo htmlspecialchars($bedNum)?></span>
                        ...
                        <div class="container">
                            ...
                            <div class="custom_btn" id="bath_num">
                                ...
                                <span id="num_toilettes"><?php echo htmlspecialchars($bedNum)?></span>
               ...
               <div class="container">
                   ...
                   <ul>
                       <li><input type="checkbox" name="quirks[]" 
                            value="Meublé" 
                            <?php if(empty($_POST['quirks'])){if(in_array('Meublé',$quirks)){echo 'checked';}}
                                  if(!empty($_POST['quirks'])){if(in_array('Meublé',$_POST['quirks']))echo 'checked';}
                       ...
                       <li><input type="checkbox" name="quirks[]" 
                            value="Non-meublé" 
                            <?php if(empty($_POST['quirks'])){if(in_array('Non-meublé',$quirks)){echo 'checked';}}
                                  if(!empty($_POST['quirks'])){if(in_array('Non-Meublé',$_POST['quirks']))echo 'checked';}  
                            ?>
                            >Non-Meublé
                      </li>
                      <li><input type="checkbox" name="quirks[]" 
                            value="Climatisation"
                            <?php if(empty($_POST['quirks'])) if(in_array('Climatisation',$quirks)){echo 'checked';}
                                  if(!empty($_POST['quirks'])){if(in_array('Climatisation',$_POST['quirks']))echo 'checked';}  
                            ?>>
                            Climatisation
                        </li>
                       <li><input type="checkbox" name="quirks[]" 
                            value="Gardiens" 
                            <?php if(empty($_POST['quirks'])){if(in_array('Gardiens',$quirks)){echo 'checked';}}
                                  if(!empty($_POST['quirks'])){if(in_array('Gardiens',$_POST['quirks']))echo 'checked';}
                            ?>>
                            Gardiens
                        </li>
                       <li><input type="checkbox" name="quirks[]" 
                            value="Machine à laver"
                            <?php if(empty($_POST['quirks'])){if(in_array('Machine à laver',$quirks)){echo 'checked';}}
                                  if(!empty($_POST['quirks'])){if(in_array('Machine à laver',$_POST['quirks']))echo 'checked';}
                       ...
                       <li><input type="checkbox" name="quirks[]" 
                            value="Piscine"
                            <?php if(empty($_POST['quirks'])){if(in_array('Piscine',$quirks)){echo 'checked';}}
                                  if(!empty($_POST['quirks'])){if(in_array('Piscine',$_POST['quirks']))echo 'checked';}
                            ?>>
                            Piscine
                        </li>
                       <li><input type="checkbox" name="quirks[]" 
                            value="Parking"
                            <?php if(empty($_POST['quirks'])){if(in_array('Parking',$quirks)){echo 'checked';}}
                                  if(!empty($_POST['quirks'])){if(in_array('Parking',$_POST['quirks']))echo 'checked';} 
                            ?>>
                            Parking
                        </li>
                       <li><input type="checkbox" name="quirks[]" 
                            value="Groupe Electrogène" 
                            <?php if(empty($_POST['quirks'])){if(in_array('Groupe Electrogène',$quirks)){echo 'checked';}}
                                  if(!empty($_POST['quirks'])){if(in_array('Groupe Electrogène',$_POST['quirks']))echo 'checked';}
                            ?>>
                            Groupe Electrogène
                        </li>
                   </ul>
